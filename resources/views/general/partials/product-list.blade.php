@foreach ($products as $product)
    <a href="{{ route('general.product-detail', $product->slug) }}" class="bg-white rounded-lg border border-[#DEE3ED]">
        <div class="relative w-full aspect-square">
            ...
            @endif
            <img src="{{ Storage::url($product->images->first()->image_path) }}" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-t">
        </div>
